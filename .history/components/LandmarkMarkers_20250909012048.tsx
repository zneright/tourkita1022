    ...
    const onPointPress = (event: OnPressEvent) => {
        try {
            ...
            if (landmark.category === "Event") {
                if (Array.isArray(landmark.events) && landmark.events.length > 1) {
                    setEventList(landmark.events); // open list modal
                } else {
                    setSelectedEvent(landmark); // open detail modal
                }
            } else {
                setSelectedLandmark(landmark); // normal markers → open LandmarkSheet
            }
        } catch (error) {

...
export default function SelectedLandmarkSheet() {
    ...
    const [loadingShezet, setLoadingSheet] = useState(true);
    const navigation = useNavigation<NavigationProp>();

    const handleView3D = async () => {
        // This function can now be simpler if you prefer. 
        // We just need to navigate; the View3D screen will handle the rest.
        try {
    ...
    const handleViewRelics = async () => {
        // This function already correctly fetches the list of relics.
        // It just needs to navigate to the RelicList screen, which will now handle downloads.
        try {
            const q = query(
            ...
            if (relics.length === 0) {
                Alert.alert("No Relics Found", `No relics listed for ${selectedLandmark.name}.`);
                return;
            }
    ...
    const handleViewAR = async () => {
        // This navigation is perfect. It passes the ID, and the ArCam screen handles the rest.
        try {

...
export function CameraPan() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.error("ARCam Download failed:", e);
    } finally {
  ...
  if (isLoading) {
    return <View style={styles.centered}><ActivityIndicator size="large" color="#007AFF" /><Text>Checking Assets...</Text></View>;
  }

  if (!arData) {
    return <View style={styles.centered}><Text>AR content not available for: {arTargetId}</Text></View>;
  }

  if (isDownloaded) {
    return (
      <View style={styles.container}>
        <ViroARSceneNavigator autofocus initialScene={{ scene: () => <ARScene {...arData} {...localUris} targetName={arData.name} /> }} worldAlignment="Gravity" />
        <Button />
      </View>
    );
  }

...
export default function SelectedLandmarkSheet() {
    ...
    useEffect(() => {
        if (selectedLandmark) {
            ...
            setResources([]);
            setDownloadStatus({});
            Promise.all([fetchAverageRating(), checkEvents(), fetchDownloadableResources()])
    ...
    const fetchDownloadableResources = async () => {
        ...
        try {
            const q = query(
                collection(db, "landmarkResources"),
                where("landmarkName", "==", selectedLandmark.name)
            );
            const snapshot = await getDocs(q);
            const fetchedResources = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setResources(fetchedResources);
            await checkAllFilesStatus(fetchedResources); // Check their status after fetching
        } catch (error) {
            console.error("Error fetching resources:", error);
        }
    };

    const checkAllFilesStatus = async (resourcesToCheck) => {
        const statusMap = {};
        const landmarkDir = getSafeDirName(selectedLandmark.name);

        for (const resource of resourcesToCheck) {
            const filename = getFilenameFromUrl(resource.resourceUrl);
            const localUri = `${SHARED_ASSETS_DIR}${landmarkDir}/${filename}`;
            const fileInfo = await FileSystem.getInfoAsync(localUri);

            statusMap[resource.id] = {
                localUri,
                status: fileInfo.exists ? 'downloaded' : 'not_downloaded',
                progress: fileInfo.exists ? 1 : 0,
            };
        }
    ...
    const handleDownload = async (resource) => {
        const { id, resourceUrl } = resource;
        const localUri = downloadStatus[id]?.localUri;
        if (!localUri) return;

        setDownloadStatus(prev => ({ ...prev, [id]: { ...prev[id], status: 'downloading', progress: 0 } }));
        ...
        const downloadResumable = FileSystem.createDownloadResumable(
            ...
            (progress) => {
                ...
                setDownloadStatus(prev => ({ ...prev, [id]: { ...prev[id], progress: p } }));
            }
        );

        try {
            await downloadResumable.downloadAsync();
            setDownloadStatus(prev => ({ ...prev, [id]: { ...prev[id], status: 'downloaded', progress: 1 } }));
        } catch (e) {
            console.error(e);
            setDownloadStatus(prev => ({ ...prev, [id]: { ...prev[id], status: 'not_downloaded' } }));
        }
    };

    const handleDelete = async (resourceId) => {
        const localUri = downloadStatus[resourceId]?.localUri;
        if (!localUri) return;
        ...
        try {
            ...
            setDownloadStatus(prev => ({
                ...prev,
                [resourceId]: { ...prev[resourceId], status: 'not_downloaded', progress: 0 },
            }));
            Alert.alert("Resource Deleted", "The downloaded file has been removed.");
        } catch (e) {
    ...
    const handleView = (resource) => {
        const localUri = downloadStatus[resource.id]?.localUri;
        if (resource.resourceType === 'video') {
            // Here you would navigate to a video player screen or open a modal
            Alert.alert("Play Video", `Now playing from: ${localUri}`);
        } else {
            // For PDFs, etc., use Linking
            Linking.openURL(localUri).catch(() => Alert.alert("Error", "Cannot open this file type."));
        }
    };

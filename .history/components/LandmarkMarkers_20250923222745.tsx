...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
    ...
    const points = landmarks
        .filter((l) => {
            const iconKey = getIconKey(l);
            if (!iconKey) return false; // skip unknown categories
            if (!l.latitude || !l.longitude) return false; // skip invalid coords

            if (selectedCategory === "All") return true;
            if (selectedCategory === "Events") return l.category === "Event";
            if (selectedCategory === "Restroom") return l.accessibleRestroom === true;

...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
    ...
    const getIconKey = (landmark: any) => {
        ...
        if (name.includes("museum") || category === "museum" || option === "museum") return "museum";
        if (name.includes("historical") || category === "historical" || option === "historical") return "historical";
        if (name.includes("government") || category === "government" || option === "government") return "government";
        if (name.includes("park") || category === "park" || option === "park") return "park";
        if (name.includes("food") || category === "food" || option === "food") return "food";
        if (name.includes("school") || category === "school" || option === "school") return "school";

        // Unknown category → no icon
        return null;
    };
    ...
    const points = filtered
        .map((landmark, index) => {
            const iconKey = getIconKey(landmark);
            if (!iconKey) return null; // skip unknown categories
            return point([landmark.longitude, landmark.latitude], {

...
const EventCalendar = () => {
    ...
    const tomorrowStr = formatDate(new Date(new Date().setDate(new Date().getDate() + 1)));

    // Filter events for today & tomorrow
    const todayTomorrowEvents = events.filter((event) => {
        if (!event.startDate) return false;
        return event.startDate === todayStr || event.startDate === tomorrowStr;
    });
        ...
        const fetchEvents = async () => {
            ...
            try {
                ...
                eventSnap.forEach((doc) => {
                    ...
                    const startDate = data.startDate ?? data.date ?? formatDate(new Date());
            ...
            {loading ? (
                ...
                <ScrollView contentContainerStyle={styles.scrollContainer}>
                    {todayTomorrowEvents.map((event) => {
                        const label = event.startDate === todayStr ? "Today" : "Tomorrow";

...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
    ...
    const getIconKey = (landmark: any): string | null => {
    ...
    // Filter landmarks strictly by selected category and valid coordinates
    const points = landmarks
        .map((landmark, index) => {
            ...
            // Only allow landmarks with a recognized icon for the current category
            const iconKey = getIconKey(landmark);
            if (!iconKey) return null;

            // Only include if it matches the selectedCategory
            if (selectedCategory === "All") {
                // include only landmarks with a proper icon
            } else if (selectedCategory === "Events") {
                if (landmark.category !== "Event") return null;
            } else if (selectedCategory === "Restroom") {
                if (!landmark.accessibleRestroom) return null;
            } else if (
                landmark.category !== selectedCategory &&
                landmark.categoryOption !== selectedCategory
            ) {
                return null;
            }

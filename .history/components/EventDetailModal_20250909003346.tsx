...
interface Props {
    ...
    onNavigate: (coords: { latitude: number; longitude: number; name: string }) => void;
}
...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event, onNavigate }) => {
    ...
    useEffect(() => {
        const fetchLocation = async () => {
            ...
            if (event.customAddress && event.lat && event.lng) {
                // Use event-provided coordinates
                setCoords({ latitude: event.lat, longitude: event.lng });
            } else if (event.locationId) {
                try {
                    const markerDoc = await getDoc(doc(db, "markers", String(event.locationId)));
                    if (markerDoc.exists()) {
                    ...
                    console.error("Error fetching marker location for event:", e);
                }
            }
        };

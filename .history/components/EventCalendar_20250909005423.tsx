...
const EventCalendar = () => {
    ...
    const todayTomorrowEvents = events.filter((event) => {
        ...
        const todayDay = format(today, "eee").toLowerCase();    // "mon"
        const tomorrowDay = format(tomorrow, "eee").toLowerCase(); // "tue"

        // --- Handle recurrence types ---
        if (recurrenceType === "once") {
            // Single event, just check normal interval
            return (
        ...
        // fallback for events with no recurrence
        return (
            isWithinInterval(today, { start, end }) ||
            isWithinInterval(tomorrow, { start, end })
        );

    ...
    const checkEvents = async () => {
        ...
        try {
            ...
            snapshot.forEach((docSnap) => {
                ...
                const recurrence = ev.recurrence || { frequency: "once" };
                const recurrenceDays: number[] =
                    recurrence.daysOfWeek?.map((d: string) => weekdayMap[d.toLowerCase()]) || [];

                let current = new Date(startDate);
                while (current <= endDate) {
                    // 👇 Only add event if current is within this week
                    // AND matches recurrence rules
                    const matchesRecurrence =
                        recurrence.frequency === "once"
                            ? current.getTime() === startDate.getTime()
                            : recurrence.frequency === "weekly"
                                ? recurrenceDays.includes(current.getDay())
                                : false;

                    if (
                        matchesRecurrence &&
                        isWithinInterval(current, { start: weekStart, end: weekEnd })
                    ) {

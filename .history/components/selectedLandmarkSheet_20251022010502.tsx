                        ...
                        {selectedLandmark.arCameraSupported && (
                            <View style={{ alignItems: 'center', flexDirection: 'row', justifyContent: "center", gap: 20, marginTop: 10 }}>
                                ...
                                <View style={{ flexDirection: 'column', alignItems: "center" }}>
                                    ...
                                    <TouchableOpacity
                                        style={styles.button}
                                        // Inside the onPress for the "View in Real World" button
                                        onPress={async () => {
                                            try {
                                                bottomSheetRef.current?.close(); // Close the bottom sheet first

                                                // The document ID is simply the landmark's name
                                                const targetId = selectedLandmark.name;
                                                const ref = doc(db, "arTargets", targetId);
                                                const snap = await getDoc(ref);

                                                if (!snap.exists()) {
                                                    console.warn(`No AR target found for: ${targetId}`);
                                                    // You can navigate to an error state or show an alert
                                                    return;
                                                }

                                                // Navigate to ArCamScreen and pass ONLY the ID

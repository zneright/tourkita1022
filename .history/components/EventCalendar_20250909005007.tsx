...
const EventCalendar = () => {
    ...
    const todayTomorrowEvents = events.filter((event) => {
        ...
        // Base check for normal events within date range
        const isNormalEvent =
        ...
        if (event.recurrence?.frequency) {
            ...
            if (event.recurrence.frequency === "daily") {
                // Occurs every day, but still within start-end
                return (
            ...
            if (event.recurrence.frequency === "weekly" && daysOfWeek.length > 0) {
                const occursToday = daysOfWeek.includes(todayDay) && today >= start && today <= end;
                const occursTomorrow = daysOfWeek.includes(tomorrowDay) && tomorrow >= start && tomorrow <= end;
                return occursToday || occursTomorrow;
            }

            // Future: add monthly/yearly if needed
        }

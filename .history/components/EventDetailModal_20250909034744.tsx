...
const EventDetailModal: React.FC<Props> = ({ visible, onClose, event }) => {
    ...
    useEffect(() => {
        if (event?.locationId && typeof event.locationId === "string") {
            const fetchMarkerName = async () => {
                try {
                    const docRef = doc(db, "markers", event.locationId as string);
                    const snap = await getDoc(docRef);
                    if (snap.exists()) {
                        setMarkerName(snap.data().name);
                    }
                } catch (err) {
                    console.error("Failed to fetch marker name:", err);
                }
            };
            fetchMarkerName();
        }
    ...
    const handleNavigate = async () => {
        try {
            ...
            if (event.lat && event.lng) {
                ...
                const snap = await getDoc(docRef);
                if (snap.exists()) {
                    const data = snap.data() as { latitude: number; longitude: number; [key: string]: any };
                    const marker = { id: snap.id, ...data };
                    setSelectedLandmark(marker);
                    navigation.navigate("Maps", {
                        latitude: data.latitude,
                        longitude: data.longitude,
                        name: event.title, // <-- use event.title instead of marker.name
                    });
                } else {
                    navigation.navigate("Maps");
                }
                    ...
                    <ScrollView contentContainerStyle={styles.content}>
                        ...
                        <View style={[styles.cardBase, styles.infoCard]}>
                            ...
                            <Text style={styles.infoText} numberOfLines={2}>
                                {event.customAddress?.label || markerName || event.address || "No address provided"}
                            </Text>
                        </View>

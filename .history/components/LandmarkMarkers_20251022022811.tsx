...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
            ...
            try {
                ...
                if (selectedCategory === "Events") {
                    ...
                    for (const docSnap of snapshot.docs) {
                        ...
                        // ✅ **FIX 1: VALIDATE EVENT COORDINATES**
                        // Only add the event if its coordinates are valid numbers.
                        if (typeof lat === 'number' && typeof lng === 'number' && !isNaN(lat) && !isNaN(lng)) {
                            fetched.push({ ...data, latitude: lat, longitude: lng, category: "Event", occursToday });
                        } else {
                            console.warn(`Skipping event "${data.title}" due to missing or invalid coordinates.`);
                        }
                    }
                    setLandmarks(fetched);
                } else {
                    const snapshot = await getDocs(collection(db, "markers"));
                    // ✅ **FIX 2: VALIDATE MARKER COORDINATES**
                    // Map, validate, and then filter out any markers with invalid coordinates.
                    ...
                    }).filter(Boolean); // This removes all null items from the array
    ...
    const filtered = landmarks.filter((l) => {
        ...
        if (selectedCategory === "Events") return true; // Already pre-filtered by the fetch

...
type EventType = {
    ...
    customAddress?: { lat: number; longitude: number; label: string };
    locationId?: string;
};
    ...
    const handleNavigate = async () => {
        try {
            ...
            if (event.customAddress) {
                const target = {
                    latitude: event.customAddress.lat,
                    longitude: event.customAddress.long,
                    name: event.title,
                };

                // Set the landmark temporarily so loadDirection works
                ...
                // Navigate to map AFTER setting landmark and loading direction
                navigation.navigate("Maps", target);

            } else if (event.locationId) {
                // Marker events: navigate normally, do not load directions

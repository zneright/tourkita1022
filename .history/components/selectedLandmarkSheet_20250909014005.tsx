...
export default function SelectedLandmarkSheet() {
    ...
    // Fix: add default value to useLandmark to avoid {} type
    ...
    useEffect(() => {
        const resolveAddress = async () => {
            ...
            // 1. Custom address exists → use it
            if (selectedLandmark.customAddress?.trim()) {
                setResolvedAddress(selectedLandmark.customAddress);
                return;
            }
            if (selectedLandmark) {
                ...
                loadDirection();
                // 2. Event with locationId → fetch marker address
                if (selectedLandmark.category === "Event" && selectedLandmark.locationId) {
                    try {
                        const markerDoc = await getDocs(
                            query(collection(db, "markers"), where("id", "==", String(selectedLandmark.locationId)))
                        );
                        if (!markerDoc.empty) {
                            const markerData = markerDoc.docs[0].data();
                            setResolvedAddress(markerData.address || "No address available");
                            return;
                        }
                    } catch (err) {
                        console.error("Error fetching marker address:", err);
                    }
                }

                // 3. Fallback
                setResolvedAddress(selectedLandmark.address || "No address available");
            };
...
const checkEvents = async () => {
    ...
    try {
        ...
        snapshot.forEach((docSnap) => {
            ...
            // Parse dates
            const startDate = ev.startDate?.toDate?.() || new Date(ev.startDate);
            const endDate = ev.endDate?.toDate?.() || new Date(ev.endDate);

            if (!startDate || !endDate || isNaN(startDate.getTime()) || isNaN(endDate.getTime())) return;

            // Repeating weekly events → extend forward
            let current = new Date(startDate);
            while (current <= endDate) {
                // check only this week’s range
                if (isWithinInterval(current, { start: weekStart, end: weekEnd })) {
                    ...
                    // If today matches and within hours → closed for private event
                    const sameDay =
                ...
                // increment → next week if repeating
                if (ev.repeat) {
...
const isOpenToday = () => {
    ...
    // === New: treat 00:00 - 23:59 as 24-hour open ===

...
export default function LandmarkMarkers({ selectedCategory, onLoadingChange }: any) {
    ...
    const onPointPress = async (event: OnPressEvent) => {
        try {
            ...
            const isEvent = selectedCategory === "Events" || landmark.category === "Event";
            const hasCustomAddress = landmark.customAddress && landmark.customAddress.trim() !== "";

            if (isEvent && hasCustomAddress) {
                // For events with a custom address, just navigate
                setSelectedLandmark({
                    ...landmark,
                    latitude: parseFloat(landmark.latitude),
                    longitude: parseFloat(landmark.longitude),
                });
                loadDirection(); // triggers your map direction/navigation
                return;
            }

            if (isEvent) {
                // handle single or multiple events without customAddress
                ...
                if (eventsHere.length === 1) {
                    const eventItem = eventsHere[0];

                    if (!eventItem.customAddress && eventItem.locationId) {
                        try {
                            const markerDoc = await getDoc(doc(db, "markers", String(eventItem.locationId)));
                            if (markerDoc.exists()) {
                                eventItem.address = markerDoc.data().address || "";
                            }
                        } catch (e) {
                        ...
                        eventItem.address = eventItem.customAddress || "";
                    }
                ...
                // normal landmarks (without custom address)

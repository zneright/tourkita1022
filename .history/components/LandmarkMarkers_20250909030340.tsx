...
// Make sure EventListModal.tsx exists in the same folder, or update the path if it's located elsewhere.
        ...
        try {
            ...
            if (selectedCategory === "Events" || landmark.category === "Event") {
                ...
                if (eventsHere.length === 1) {
                    const event = eventsHere[0];
                    // resolve address if needed
                    if (!event.customAddress && event.locationId) {
                        try {
                            const markerDoc = await getDoc(doc(db, "markers", String(event.locationId)));
                            if (markerDoc.exists()) {
                                event.address = markerDoc.data().address || "";
                            }
                        } catch (e) {
                        ...
                        event.address = event.customAddress || "";
                    }

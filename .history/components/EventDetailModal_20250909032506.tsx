...
a const handleNavigate = async () => {
        try {
            ...
            if (event.customAddress) {
                ...
                // Load directions only for customAddress
                loadDirection({
                    latitude: event.customAddress.latitude,
                    longitude: event.customAddress.longitude,
                });
                ...
                // Fetch marker coordinates but DO NOT load directions
                const docRef = doc(db, "markers", event.locationId);
                const snap = await getDoc(docRef);
                if (snap.exists()) {
                    const target = { id: snap.id, ...snap.data() };
                    setSelectedLandmark(target); // only for marker events
                    navigation.navigate("Maps", {

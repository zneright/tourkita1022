...
export default function SplashScreen({ navigation }: any) {
    ...
    useEffect(() => {
        ...
        const authUnsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
            setUser(firebaseUser);   // Set the user (or null) in your global state
    ...
    useEffect(() => {
        ...
        const navigationTimeout = setTimeout(() => {
            if (isConnected) {
                // If connected, navigate based on whether a user object exists
    ...
    }, [isConnected, isAuthChecked, user, navigation]); // This effect re-runs when these state values change

    // Interpolate animation values for a combined fade and slide effect
    const slideUp = contentAnim.interpolate({
        inputRange: [0, 1],
        outputRange: [20, 0], // Starts 20px down and moves up to its final position
    });
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.mainContent}>
                ...
                {/* The <Animated.Text> component has been removed */}
            </View>
...
const styles = StyleSheet.create({
    container: {
        ...
        alignItems: 'center',
        backgroundColor: '#F9F4F0', // A clean, warm off-white
    },
    mainContent: {
        flex: 1,
        justifyContent: 'center',
    ...
    appLogo: {
        width: 220, // Logo is now larger to be the main focus
        height: 220,
        borderRadius: 50, // Adjusted for the new size
    },
    // The `appName` style has been removed
    footerContainer: {
        position: 'absolute',
        bottom: 60,
        alignItems: 'center',

...
const SearchScreen = () => {
    ...
    const isClosedDueToPrivateEvent = (markerId: string): boolean => {
        ...
        const relevantEvents = events.filter((event) => {
            if (!event) return false; // skip null or undefined
            if (!event.startDate) return false; // skip events with no startDate
  ...
  .map(doc => {
    const d = doc.data();
    if (!d) return null; // skip invalid docs
    return {
      id: doc.id,
      title: d.title ?? "Untitled Event",
      description: d.description,
      imageUrl: d.imageUrl,
      address: d.customAddress?.trim() || markersData.find(m => m.id === d.locationId)?.name || "Address not available",

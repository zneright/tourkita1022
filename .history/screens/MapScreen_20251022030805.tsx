...
export default function MapsScreen() {
    ...
    // All of your original useEffect and handler functions remain unchanged
    useEffect(() => {
    ...
    const toggleNavigation = () => {
        if (navigationMode) {
            ...
            cameraRef.current?.setCamera({
                centerCoordinate: undefined, zoomLevel: 15, pitch: 0, heading: 0, animationDuration: 1000,
            });
        } else {
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.container}>
                <MapView style={styles.map} styleURL={currentMap} rotateEnabled>
                    <Camera
                        ref={cameraRef}
                        defaultSettings={{ centerCoordinate: [120.9754, 14.5912], zoomLevel: 15 }}
                    ...
                    <LandmarkMarkers selectedCategory={selectedCategory} onLoadingChange={setIsLoading} />
                    {showDirection && directionCoordinates && <LineRoute coordinates={directionCoordinates} />}
                </MapView>
                ...
                <View style={styles.mapControlsContainer}>
                    <TouchableOpacity style={styles.controlButton} onPress={() => cameraRef.current?.fitBounds(MANILA_BOUNDS.ne, MANILA_BOUNDS.sw, 50, 1000)}>
                        <FontAwesome5 name="city" size={18} color="#4E342E" />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.controlButton} onPress={() => cameraRef.current?.fitBounds(PH_BOUNDS.ne, PH_BOUNDS.sw, 50, 1000)}>
                        <FontAwesome5 name="globe-asia" size={18} color="#4E342E" />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.controlButton} onPress={() => setVisible(!visible)}>
                        <Ionicons name={visible ? "eye-off-outline" : "eye-outline"} size={20} color="#4E342E" />
                    </TouchableOpacity>
                    <View style={[styles.controlButton, { paddingVertical: 8 }]}>
                        <Ionicons name={currentMap.includes('satellite') ? "satellite-outline" : "map-outline"} size={20} color="#4E342E" />
                        <Switch
                ...
                <TouchableOpacity onPress={toggleNavigation} style={[styles.navigationButton, { backgroundColor: navigationMode ? "#D32F2F" : "#493628" }]}>
                    ...
                    <Text style={styles.navigationButtonText}>{navigationMode ? "Exit" : "Start Navigation"}</Text>
                </TouchableOpacity>
                ...
                {(showDirection || !showBottomNav) && (
                    <View style={[styles.infoPanel, !showBottomNav && styles.infoPanelExpanded]}>
                        <View style={styles.infoItem}>
                            ...
                            <Text style={styles.infoText}>{distance != null ? `${(distance / 1000).toFixed(2)} km` : "-- km"}</Text>
                        </View>
                        <View style={styles.infoSeparator} />
                        <View style={styles.infoItem}>
                            <Entypo name="back-in-time" size={18} color="#4E342E" />
                            <Text style={styles.infoText}>{duration != null ? `${(duration / 60).toFixed(0)} min` : "-- min"}</Text>
                        </View>
                    </View>
                )}
...
const styles = StyleSheet.create({
    ...
    infoPanel: {
        position: "absolute",
        bottom: 160, // Positioned above the navigation button
        alignSelf: "center",
        flexDirection: "row",
    ...
    infoPanelExpanded: {
        bottom: 20, // Lower position when bottom nav is hidden
    },
    infoItem: {
        flexDirection: "row",
        alignItems: "center",

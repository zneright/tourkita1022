...
// Import our shared file system helpers
import { SHARED_ASSETS_DIR, getFilenameFromUrl, getSafeDirName } from '../utils/fileSystem';

// --- NEW: A sub-component for each item in the list ---
const RelicItem = ({ item, navigation }) => {
    ...
    // Construct the local file path using the shared directory structure
    const localUri = `${SHARED_ASSETS_DIR}${getSafeDirName(item.locationName)}/${getFilenameFromUrl(item.modelUrl)}`;

    // Check the file status when the component mounts
    useEffect(() => {
    ...
    const handleView = () => {
        if (status === 'downloaded') {
            navigation.navigate("View3D", {
                modelUrl: item.modelUrl, // Pass original URL for reference
                title: item.name, // The title is used to find the shared local file
            });
        }
    };
...
const styles = StyleSheet.create({
    ...
    actionButton: { padding: 8, borderRadius: 20, marginHorizontal: 4 },

...
export default function View3D({ route }) {
    ...
    return (
        <View style={styles.container}>
            {/* 3. ADD the header with a back button and title */}
            <View style={styles.header}>
                <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
                    <Text style={styles.backButtonText}>{'‹'}</Text>
                </TouchableOpacity>
                <Text style={styles.title}>{title || "3D Viewer"}</Text>
                <View style={styles.spacer} /> {/* This empty view helps center the title */}
            </View>
...
// 4. ADD styles for the new header
const styles = StyleSheet.create({
    ...
    title: {
        flex: 1, // Allows title to take up space and center itself
        textAlign: 'center',
        fontSize: 22,
        fontWeight: 'bold',
        color: '#4E342E',
    ...
    spacer: {
        width: 40, // Balances the back button to keep the title perfectly centered
    },

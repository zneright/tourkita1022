...
const SearchScreen = () => {
    ...
    useEffect(() => {
        const fetchMarkersAndEvents = async () => {
            ...
            try {
                // Fetch markers first
                const snapshotMarkers = await getDocs(collection(db, "markers"));
                const markersData: Marker[] = snapshotMarkers.docs.map((doc) => {
                    const d = doc.data();
                    return {
                        id: doc.id,
                        name: d.name,
                ...
                // Convert markers array to a map for quick lookup
                ...
                snapshotEvents.docs.forEach((doc) => {
                    const d = doc.data();

                    const startDate = d.startDate ? parseISO(d.startDate) : new Date();
                    const endDate = d.endDate ? parseISO(d.endDate) : startDate;

                    const today = new Date();
                    const tomorrow = addDays(today, 1);
                    const occursTodayOrTomorrow = (date: Date) => isToday(date) || isTomorrow(date);

                    // Determine address: customAddress > marker address
                    let address = d.customAddress;
                    if (!address && d.locationId && markersMap[d.locationId]) {
                        address = markersMap[d.locationId].name || markersMap[d.locationId].address;
                    }
                    if (!address) address = "Address not available";

                    // Handle recurring events
                    if (d.recurrence?.frequency === "weekly" && d.recurrence.daysOfWeek) {
                        ...
                        const eventDays = d.recurrence.daysOfWeek.map((day: string) => daysMap[day]);

...
const CalendarViewScreen = () => {
    ...
    const occursOnDate = (event: Event, target: Date) => {
        ...
        const recurrenceType = event.recurrence?.frequency?.toLowerCase() || "once";
        const daysOfWeek = (event.recurrence?.daysOfWeek || []).map((d) =>
            d.toLowerCase().slice(0, 3)
        );

        const targetDay = format(target, "eee").toLowerCase(); // "mon","tue"
...
fetchedEvents.forEach((event) => {
    ...
    const finalDate = event.endDate ? parseISO(event.endDate) : current;
        ...
        const fetchEvents = async () => {
            ...
            try {
                ...
                eventSnap.forEach((doc) => {
                    ...
                    const eventStartTime: string = data.eventStartTime ?? "";
                    const eventEndTime: string = data.eventEndTime ?? "";
                    const startDate: string = data.startDate ?? data.date ?? "";
    ...
    const onDayPress = (day: any) => {
        ...
        const filtered = events.filter((e) => {
            const start = parseISO(e.startDate!);
            const end = parseISO(e.endDate || e.startDate!);
            const target = parseISO(selected);
            return target >= start && target <= end;
        });

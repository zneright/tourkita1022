...
export default function View3D({ route }) {
        ...
        const checkIfDownloaded = async () => {
            if (localUri) {
                ...
                if (fileInfo.exists) {
                    ...
                    console.log('3DView: Model already downloaded at:', localUri);
                } else {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const downloadResumable = FileSystem.createDownloadResumable(
                modelUrl, localUri, {}, (progress) => {
                    setDownloadProgress(progress.totalBytesWritten / progress.totalBytesExpectedToWrite);
                }
            );

            const { uri } = await downloadResumable.downloadAsync();
            console.log('3DView: Finished downloading to ', uri);
            setIsDownloaded(true);
        } catch (e) {
            console.error("3DView Download Error:", e);
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.log('3DView: Deleted model:', localUri);
            setIsDownloaded(false);
        } catch (e) {
            console.error('3DView: Error deleting file', e);
        }
    };
        ...
        if (!modelUrl) {
            return (
                <View style={[styles.modelView, styles.centered]}>
                    <Text style={styles.infoText}>No 3D model provided.</Text>
                </View>
            );
        }
            ...
            {isDownloaded && modelUrl && (
                <View style={styles.buttonContainer}>
                    <TouchableOpacity style={[styles.button, styles.deleteButton]} onPress={handleDelete}>
                        <Text style={styles.buttonText}>Delete All Assets</Text>
                    </TouchableOpacity>
                </View>
            )}
        </View>
...
const styles = StyleSheet.create({
    container: { flex: 1, backgroundColor: '#F9F4EF', paddingTop: 30 },
    modelView: { width: '100%', height: 500 },
    centered: { justifyContent: 'center', alignItems: 'center' },
    title: { fontSize: 22, fontWeight: 'bold', color: '#4E342E', marginBottom: 10, textAlign: 'center' },

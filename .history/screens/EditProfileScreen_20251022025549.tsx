...
export default function ViewProfileScreen() {
    ...
    useEffect(() => {
        ...
        // Add a listener to refetch data every time the screen comes into focus
    ...
    const fetchUserData = async () => {
        // --- CACHE-FIRST LOGIC ---
        // 1. Try to load from cache immediately for a fast UI response.
        try {
            const cached = await AsyncStorage.getItem('cachedUserData');
            if (cached) {
                setUserData(JSON.parse(cached));
                setLoading(false); // Stop initial loader if cache exists
            }
        } catch (cacheError) {
            console.error('Error reading from cache:', cacheError);
        }

        // 2. Then, fetch fresh data from Firestore to update the UI and the cache.
        try {
            const auth = getAuth();
            ...
            if (!querySnapshot.empty) {
                ...
                setUserData(data); // Update state with fresh data
                await AsyncStorage.setItem('cachedUserData', JSON.stringify(data)); // Update cache
            } else {
                throw new Error("User not found in Firestore.");
            }
        } catch (error) {
            console.error('Error fetching from Firestore:', error);
            // If Firestore fails and we still don't have cached data, stop loading
            if (!userData) setLoading(false);
        } finally {
            // Ensure loading is always stopped eventually
            setLoading(false);
        }
    };

    // Use useMemo to prevent recalculating the name on every render
            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                <View style={styles.headerCard}>
                    <View style={styles.avatarContainer}>
                        {loading && !userData ? ( // Show skeleton only on the very first load
                ...
                {(!loading || userData) && ( // Show button once loading is done or if there's cached data
                    <TouchableOpacity
...
const styles = StyleSheet.create({
    ...
    infoCard: {
        backgroundColor: '#FFFFFF',
        borderRadius: 16,
        paddingHorizontal: 20, // Use horizontal padding for cleaner borders
        elevation: 3,
        shadowColor: '#000',
        shadowOpacity: 0.1,
        shadowRadius: 10,
    ...
    infoRowLast: { // Add this to remove the border on the last item
        borderBottomWidth: 0,
    },
    infoTextContainer: {
        marginLeft: 16,

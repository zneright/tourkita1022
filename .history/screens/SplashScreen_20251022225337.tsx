...
import { useUser } from "../context/UserContext"; // Your user context from previous steps
...
export default function SplashScreen({ navigation }: any) {
    ...
    const [isAuthChecked, setIsAuthChecked] = useState(false); // NEW: Track if Firebase check is done
    const { user, setUser } = useUser(); // NEW: Get user from your global context

    // Effect for starting animations and setting up listeners
    useEffect(() => {
        // Run animations (no change here)
        Animated.parallel([
    ...
    useEffect(() => {
        // Don't do anything until we have checked for connection AND auth status
        if (isConnected === null || !isAuthChecked) {
            return;
        }

        // Wait for the animation to feel complete before navigating
